...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session) {
        ...
        if (admins.includes(session.user.email)) fetchAllResellers();
      }
      setLoading(false);
    });
  ...
  const handleRenewReseller = async (resellerId) => {
    let d = new Date();
    d.setDate(d.getDate() + 30);
    const { error } = await supabase.from('config_max').update({ data_vencimento: d.toISOString() }).eq('id', resellerId);
    if (error) alert('Erro ao renovar: ' + error.message);
    else {
      alert('Assinatura renovada por mais 30 dias!');
      fetchAllResellers();
    }
  };
