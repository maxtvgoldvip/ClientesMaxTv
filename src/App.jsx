...
export default function App() {
  ...
  // Cálculo corrigido: Bruto - Custos Fixos (5.00 por ativo P2)
  const processedClients = clients.map(c => {
    ...
    if (c.tipo === 'Revendedor') {
      ...
      custo = (c.qtd_ativos_p1 * config.custo_painel1) + (c.qtd_ativos_p2 * 5.00); // 5.00 é o custo fixo P2
    } else {
  ...
  if (!session) return <div className="text-white p-10">Faça login...</div>;
    ...
    <div className="min-h-screen bg-[#0b0f19] text-white p-6">
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-900 p-4 rounded-xl border border-gray-800">
            ...
            <h2 className="text-xl font-bold">R$ {totalBruto.toFixed(2)}</h2>
        </div>
        <div className="bg-gray-900 p-4 rounded-xl border border-gray-800">
            <p className="text-xs text-gray-400">LUCRO LÍQUIDO</p>
            <h2 className="text-xl font-bold text-green-400">R$ {totalLiquido.toFixed(2)}</h2>
        </div>
      </div>

      <button onClick={() => setShowModal(true)} className="w-full bg-amber-500 py-3 rounded-xl font-bold text-black mb-6">Adicionar Cliente</button>

      <div className="space-y-4">
        {processedClients.map(c => (
            <div key={c.id} className="bg-[#111827] p-4 rounded-xl border border-gray-800 flex justify-between">
                <div>
                    <p className="font-bold">{c.nome}</p>
                    <p className="text-xs text-gray-400">{c.painel}</p>
                </div>
                <div className="text-right">
                    <p className="font-bold text-green-400">R$ {c.liquido.toFixed(2)}</p>
                    <button onClick={() => { setEditingClient(c); setShowModal(true); }} className="text-xs text-blue-400">Editar</button>
                </div>
            </div>
        ))}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-6">
            <form onSubmit={handleSaveClient} className="bg-[#111827] p-6 rounded-2xl w-full max-w-sm border border-gray-800">
                <input placeholder="Nome" value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} className="w-full p-2 mb-2 bg-gray-900 rounded"/>
                <select value={formData.tipo} onChange={(e) => setFormData({...formData, tipo: e.target.value})} className="w-full p-2 mb-2 bg-gray-900 rounded">
                    <option>Cliente Direto</option>
                    <option>Revendedor</option>
                </select>
                {formData.tipo === 'Revendedor' ? (
                    <div className="flex gap-2">
                        <input type="number" placeholder="Ativos P1" onChange={(e) => setFormData({...formData, qtd_ativos_p1: e.target.value})} className="w-1/2 p-2 bg-gray-900 rounded"/>
                        <input type="number" placeholder="Ativos P2" onChange={(e) => setFormData({...formData, qtd_ativos_p2: e.target.value})} className="w-1/2 p-2 bg-gray-900 rounded"/>
                    </div>
                ) : (
                    <input type="number" placeholder="Valor" onChange={(e) => setFormData({...formData, valor_plano: e.target.value})} className="w-full p-2 bg-gray-900 rounded"/>
                )}
                <button type="submit" className="w-full mt-4 bg-amber-500 py-2 rounded font-bold text-black">Salvar</button>
