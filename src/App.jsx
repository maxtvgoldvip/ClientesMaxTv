...
export default function App() {
  ...
  const processed = clients.map(c => {
    const bruto = c.tipo === 'Revendedor' ? ((c.painel.includes('Painel 1') ? c.qtd_ativos_p1 : c.qtd_ativos_p2) * (c.painel.includes('Painel 1') ? config.revenda_painel1 : config.revenda_painel2)) : c.valor_plano;
  ...
  if (!session) return <div className="p-10 text-white"><input type="email" placeholder="Email" onChange={e=>setEmail(e.target.value)} className="bg-gray-800 p-2 mb-2 w-full"/><input type="password" placeholder="Senha" onChange={e=>setPassword(e.target.value)} className="bg-gray-800 p-2 mb-2 w-full"/><button onClick={()=>supabase.auth.signInWithPassword({email,password})} className="bg-amber-500 p-2 w-full">Entrar</button></div>;
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white p-4">
      <div className={`p-4 rounded-xl border mb-4 ${painel2Pago ? 'bg-green-950 border-green-500' : 'bg-amber-950 border-amber-500'}`}>
        {painel2Pago ? "PAINEL 2 PAGO!" : `Falta pagar: R$ ${saldoP2.toFixed(2)}`}
      </div>
      
      {activeTab === 'dashboard' ? (
        <>
          <button onClick={() => setShowModal(true)} className="bg-amber-500 p-3 w-full rounded-xl font-bold mb-4">Adicionar Novo</button>
          {processed.map(c => (
            <div key={c.id} className="bg-[#111827] p-4 rounded-xl mb-2 flex justify-between">
              ...
              <div className="text-right"><p className="text-green-400 font-bold">R$ {c.liquido.toFixed(2)}</p>
                <div className="flex gap-2"><Edit2 onClick={() => {setEditingClient(c); setFormData(c); setShowModal(true);}} className="w-4"/><Trash2 onClick={()=>handleDelete(c.id)} className="w-4"/></div>
              </div>
            </div>
          ))}
        </>
        ...
        <div className="space-y-4">
          <input type="number" value={config.custo_painel1} onChange={e=>setConfig({...config, custo_painel1: e.target.value})} className="bg-gray-800 p-2 w-full"/>
          <input type="number" value={config.revenda_painel1} onChange={e=>setConfig({...config, revenda_painel1: e.target.value})} className="bg-gray-800 p-2 w-full"/>
          <input type="number" value={config.custo_painel2_fixo} onChange={e=>setConfig({...config, custo_painel2_fixo: e.target.value})} className="bg-gray-800 p-2 w-full"/>
          <input type="number" value={config.revenda_painel2} onChange={e=>setConfig({...config, revenda_painel2: e.target.value})} className="bg-gray-800 p-2 w-full"/>
          <button onClick={handleSaveConfig} className="bg-amber-500 p-2 w-full">Salvar Configurações</button>
        </div>
      )}
      <button onClick={() => setActiveTab(activeTab === 'dashboard' ? 'config' : 'dashboard')} className="mt-4 text-amber-500 underline">Alternar Aba</button>
      
      {showModal && (
        <div className="fixed inset-0 bg-black/80 p-4 flex items-center justify-center">
          <form onSubmit={handleSaveClient} className="bg-gray-900 p-6 rounded-xl w-full">
            ...
            <select value={formData.painel} onChange={e=>setFormData({...formData, painel: e.target.value})} className="bg-gray-800 p-2 w-full mb-2">
              <option>Painel 1 (Sigma)</option><option>Painel 2 (Zenpanel)</option>
            </select>
            <button type="submit" className="bg-green-600 p-2 w-full">Salvar</button>
            <button type="button" onClick={()=>setShowModal(false)} className="bg-red-600 p-2 w-full mt-2">Fechar</button>
          </form>
        </div>
      )}
    </div>
